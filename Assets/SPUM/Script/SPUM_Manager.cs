...
public class SPUM_Manager : MonoBehaviour
{
    ...
    /**/
    public GameObject attack_range_prefab;
    /**/
    ...
    public void SavePrefabs()
    {
        if(_prefabUnitList.Count < _maxNumber)
        {
            ...
            /**/
            //ttObjST.gameObject.AddComponent<Rigidbody2D>();
            //ttObjST.gameObject.GetComponent<Rigidbody2D>().gravityScale = 0;
            /**/
            ...
            /**/
            // Rigidbody 추가
            tSObj.AddComponent<Rigidbody2D>();
            tSObj.GetComponent<Rigidbody2D>().gravityScale = 0;
            
            // 몸 콜라이더 추가
            BoxCollider2D tem = tSObj.AddComponent<BoxCollider2D>();
            tem.offset = new Vector2(0.0f, 0.37f);
            tem.size = new Vector2(0.45f, 0.7f);
            tem.isTrigger = true;


            // 공격범위를 위한 하위 오브젝트 + 공격범위 콜라이더 추가 + 하위 오브젝트의 transform 변경
            GameObject tem_gameobject = Instantiate(attack_range_prefab);
            tem_gameobject.transform.parent = tSObj.transform;
            tSObj.transform.GetChild(2).rotation = Quaternion.identity;
            tSObj.transform.GetChild(2).localScale = new Vector3(1.0f, 1.0f, 1.0f);
            tSObj.transform.GetChild(2).localPosition = new Vector3(0.0f, 0.0f, 0.0f);
            /**/
